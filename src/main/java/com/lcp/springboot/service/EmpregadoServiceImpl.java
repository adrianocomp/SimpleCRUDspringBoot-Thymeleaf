package com.lcp.springboot.service;

import java.util.List;
import java.util.Optional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.data.domain.Sort;
import org.springframework.stereotype.Service;

import com.lcp.springboot.model.Empregado;
import com.lcp.springboot.repository.EmpregadoRepository;

@Service
public class EmpregadoServiceImpl implements EmpregadoService {
	
	@Autowired
	private EmpregadoRepository empregadoRepository;
	
	@Override
	public List<Empregado> getAllEmployees() {
		return empregadoRepository.findAll();
	}

	@Override
	public void saveEmployee(Empregado employee) {
		this.empregadoRepository.save(employee);
		
	}

	@Override
	public Empregado getEmployeeById(long id) {
		Optional<Empregado> optional = empregadoRepository.findById(id);
		Empregado employee = null;
		if (optional.isPresent()) {
			employee = optional.get();
		} else {
			throw new RuntimeException(" Employee not found for id :: " + id);
		}
		return employee;
	}

	@Override
	public void deleteEmployeeById(long id) {
		this.empregadoRepository.deleteById(id);
		
	}

	@Override
	public Page<Empregado> findPaginated(int pageNo, int pageSize, String sortField, String sortDirection) {
		Sort sort = sortDirection.equalsIgnoreCase(Sort.Direction.ASC.name()) ? Sort.by(sortField).ascending() :
			Sort.by(sortField).descending();
		
		Pageable pageable = PageRequest.of(pageNo - 1, pageSize, sort);
		return this.empregadoRepository.findAll(pageable);
	}

}
